...
def query_local_dns_server(domainList,question_type):
    resolver = dns.resolver.Resolver()
    resolver.nameservers = local_host_ip
    answers = resolver.resolve(domainList, question_type) # provide the domain and question_type

    ip_address = answers[0].to_text()
    return ip_address
    
# Define a function to query a public DNS server for the IP address of a given domain name
def query_dns_server(domainList,question_type):
    resolver = dns.resolver.Resolver()
